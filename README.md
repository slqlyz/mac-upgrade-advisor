...
│   ├── check_consistency.py       # 一致性对账：可推导字段的存储值 vs 推导值
│   ├── selftest.py                # 引擎不变量自检：全库 600 组合扫描 11 条设计规则
...
# 用途 (按升级哲学分档, 按机型自适应显示, 可简写):
#   轻度日用 — SSD/适量内存, 不碰非常规方案 (所有机型可用)
#   黑苹果续命 — 选目标系统 (--target 12/15), 显卡按 Metal+驱动窗口选门槛卡,
#                诊断给出本机 OCLP 兼容性 (原装显卡是否缺 Metal); 官方已到
#                Tahoe 或 OCLP 无可续版本的机型不显示
#   秀肌肉 — 全部拉满 (含 OCLP 续命后的系统可达范围、内存/CPU 拉满、eGPU);
#            仅台式机 (笔记本无拉满空间, 不设此档)
#   野路子 — 彩蛋性质, 秀肌肉的严格超集 + 非常规内容 (iMac 换 E3、Titan V 撤回
#            版驱动); 无独有内容的机器不显示
# 风险: official / community (默认) / experimental; 黑苹果的显卡换装推导可靠性高,
#       community 级即显示; 野路子恒为拉满
# 注: eGPU 只在台式机的秀肌肉/野路子出现; 笔记本仅诊断中提一嘴
```
...
- 具体 CPU 型号编号（如 i5-3470S）：Apple 只写主频/核数，型号编号经 EveryMac 逐配置核验
  （含 iMac 2020 全部四颗）
- `cpu_socket`、`nvme_bootable`、`oclp_caveat`：社区拆解/实测知识（Dortania、iFixit 等）
- `max_macos`：来自 Apple 各版 macOS 的机型支持列表（不在规格页上）
- `board_id`：26/26 已补齐，来源 OCLP 源码 `datasets/smbios_data.py`
...
- **gpu_arch_support** — GPU 架构 × macOS 驱动支持区间 + Metal 支持 + 性能档位
  （门槛卡/拉满卡代表型号、MXM 可得性），按架构世代统一管理，MXM / PCIe / eGPU 共用；
  数据源为 Dortania 等 hackintosh 文档。
- **macos_versions** — macOS 版本 × 配置要求（Metal 强制线在 10.14 Mojave、OCLP 支持状态），
  与 gpu_arch_support 双向呼应；黑苹果续命的目标系统从这里选（`--target 12` / Web 下拉），
  显卡推荐按所选目标反推（选 Monterey 就不会拿 macOS 26 吓唬你）。
...
- [x] 第二阶段：分批数据采集
  - [x] 批次1：50 款机型官方规格（Apple 规格页逐条抓取校验，页面标记匹配才入库；覆盖 2009 Mac Pro 至 2020 iMac、含 iMac Pro/17吋/白色小白）
  ...
  - [x] 待补项已清：board_id 26/26（OCLP smbios 数据）；iMac 2020 CPU 型号 EveryMac 核验；
        无线网卡/光驱位托架/实验层/冲突数据已补（2012 非 Retina 与 mini 2012 的网卡
        升级因无充分成功实证暂不收录）
